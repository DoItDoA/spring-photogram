	...
	if ($(obj).text() === "팔로우 취소") {
	    ...
	    $.ajax({
            ...
            $(obj).toggleClass("blue");
      	    ...
      	    console.log("팔로우하기 실패", error);
      	});
	}
}

// (2) 구독자 정보  모달 보기
function subscribeInfoModalOpen(pageUserId, list) {
	$(".modal-subscribe").css("display", "flex");

	$.ajax({
        url:`/api/user/${pageUserId}/${list}`,
        dataType:"json"
    }).done(res=>{
        res.data.forEach((u)=>{
            const item = getSubscribeModalItem(u);
...
function getSubscribeModalItem(u) {
    ...
    if(!u.equalUserState) {
        if(u.subscribeState) {
            item += `<button class="cta" onclick="toggleSubscribe(${u.id},this)">팔로우 취소</button>`;
        } else {
            item += `<button class="cta blue" onclick="toggleSubscribe(${u.id},this)">팔로우</button>`;
        }
    }
...
function profileImageUpload(pageUserId, principalId) {
	...
	$("#userProfileImageInput").on("change", (e) => {
        ...
        const formData = new FormData(profileImageForm); // 사진만 사용시 이거 사용
